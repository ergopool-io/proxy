...
object TestNode extends TestJettyServer {
  import Servlets._
  ...
  // Routes
  private val testProxyRoute = "/test/proxy"
  private val miningCandidateRoute = "/mining/candidate"
  private val miningSolutionRoute = "/mining/solution"
  private val miningCandidateWithTxsRoute = "/mining/candidateWithTxs"
  private val walletTransactionGenerateRoute = "/wallet/transaction/generate"

  override protected val server: Server = createServer()

  override protected val handler = new ServletHandler()

  server.setHandler(handler)

  handler.addServletWithMapping(classOf[ProxyServlet], testProxyRoute)
  handler.addServletWithMapping(classOf[MiningCandidateServlet], miningCandidateRoute)
  handler.addServletWithMapping(classOf[MiningSolutionServlet], miningSolutionRoute)
  handler.addServletWithMapping(classOf[MiningCandidateWithTxsServlet], miningCandidateWithTxsRoute)
  handler.addServletWithMapping(classOf[WalletTransactionGenerateServlet], walletTransactionGenerateRoute)
  ...
  object Servlets {
    ...
    class MiningSolutionServlet extends HttpServlet {
      ...
      override protected def doPost(req: HttpServletRequest, resp: HttpServletResponse): Unit = {
        val body: String = readBody(req).replaceAll("\\s", "")
