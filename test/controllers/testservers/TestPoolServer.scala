...
object TestPoolServer extends TestJettyServer {
  import Servlets._

  override val serverPort: Int = 9002
  override val serverName: String = "Test Pool Server"

  // Routes
  private val transactionRoute = "/api/transaction.json/"
  private val configRoute = "/api/config/value.json/"
  private val solutionRoute = "/api/share.json/"
  private val headerRoute = "/api/header.json/"
  ...
  handler.addServletWithMapping(classOf[TransactionServlet], transactionRoute)
  handler.addServletWithMapping(classOf[ConfigServlet], configRoute)
  handler.addServletWithMapping(classOf[SolutionServlet], solutionRoute)
  handler.addServletWithMapping(classOf[HeaderServlet], headerRoute)
  ...
  object Servlets {
    ...
    class TransactionServlet extends HttpServlet {
      ...
      override protected def doPost(request: HttpServletRequest, response: HttpServletResponse): Unit = {
        val body: String = readBody(request).replaceAll("\\s", "")
